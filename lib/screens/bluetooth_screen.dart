...
class BluetoothScreen extends StatelessWidget {
  ...
  static bool _anyA2dpCapable(List<EspDevice> list) =>
      list.any((d) => d.a2dpSinkCapable == true);
  ...
  static Future<void> _toggleWithFeedback(
    ...
    if (total == 0) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'Seleziona almeno un dispositivo online in Dispositivi.',
          ),
        ),
      );
      return;
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text(parts.isEmpty ? 'Comando inviato.' : parts.join(' · ')),
      ),
    );
  }

  static Future<void> _ensureOnWithFeedback(
    BuildContext context,
    DeviceManager dm,
  ) async {
    final r = await dm.ensureBluetoothOnDetailed();
    if (!context.mounted) return;
    final total = r['total'] ?? 0;
    if (total == 0) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Nessun dispositivo online disponibile.')),
      );
      return;
    }
    final on = r['bt_on'] ?? 0;
      ...
      children: [
        Container(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(
              color: done ? AppColors.green : AppColors.border2,
            ),
          ),
  ...
  Widget build(BuildContext context) {
    return Consumer<DeviceManager>(
      builder: (context, dm, _) {
        ...
        final selected = dm.selectedDevices
            .where((d) => d.isOnline && !d.mac.startsWith('pending:'))
            .toList();
        final selectedA2dp = selected
            .where((d) => d.a2dpSinkCapable == true)
            .toList();
        final step1Done =
            selectedA2dp.isNotEmpty &&
            selectedA2dp.any((d) => d.mode.toLowerCase() == 'bluetooth');
                      ...
                      children: [
                        Row(
                          children: [
                            ...
                            const Spacer(),
                            Text(
                              selectedA2dp.isEmpty
                                  ? 'Seleziona device'
                                  : '${selectedA2dp.length} pronti',
                        ...
                        _step(
                          ...
                          subtitle: step1Done
                              ? 'OK: almeno un dispositivo è in mode BT'
                              : 'Tocca “Toggle BT su selezionati”',
                          done: step1Done,
                        ),
                        ...
                        _step(
                          ...
                          subtitle: step2Done
                              ? 'OK: sink A2DP avviato'
                              : 'Cerca il nome RetroWave in elenco',
                          done: step2Done,
                        ),
                        const SizedBox(height: 8),
                        _step(
                          ...
                          subtitle: step3Done
                              ? 'OK: sorgente audio collegata'
                              : 'Avvia un brano e seleziona output Bluetooth',
                          done: step3Done,
                        ),
                        const SizedBox(height: 12),
                        Wrap(
                          ...
                          children: [
                            if (!step1Done)
                              FilledButton.icon(
                                onPressed: () =>
                                    _ensureOnWithFeedback(context, dm),
                            ...
                            if (step3Done)
                              FilledButton.tonalIcon(
                                onPressed: () =>
                                    _toggleWithFeedback(context, dm),
                ...
                Padding(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      borderRadius: BorderRadius.circular(20),
                      border: Border.all(
                        color: AppColors.acc2.withValues(alpha: 0.45),
                      ),
                    ),
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                ...
                                Text(
                                  anyBt
                                      ? 'Almeno un ESP in mode=bluetooth'
                                      : 'Nessun ESP in mode bluetooth',
                                  style: const TextStyle(
                                    fontWeight: FontWeight.w600,
                                    fontSize: 13,
                                  ),
              ...
              Expanded(
                child: list.isEmpty
                    ? Center(
                        child: Text(
                          'Nessun ESP in lista',
                          style: TextStyle(color: AppColors.muted2),
                        ),
                      )
                    : ListView.builder(
              ...
              Padding(
                ...
                child: FilledButton(
                  ...
                  onPressed: list.isEmpty
                      ...
                      : () => _toggleWithFeedback(context, dm),
                  child: const Text(
                    'Invia /bluetooth agli ESP selezionati',
                    style: TextStyle(fontWeight: FontWeight.w800),
                  ),
                ),
              ),
  ...
  static String _deviceA2dpLine(EspDevice d) {
    if (d.a2dpSinkCapable == true) {
      if (d.a2dpSinkStarted) {
        final name =
            (d.a2dpPairingName != null && d.a2dpPairingName!.isNotEmpty)
            ? d.a2dpPairingName!
            : d.displayName;
        return d.a2dpConnected
            ? 'A2DP attivo · collegato · «$name»'
            : 'A2DP attivo · cerca «$name» nel telefono';
      }
